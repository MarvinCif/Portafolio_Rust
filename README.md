...
```sh
git remote add origin https://github.com/MarvinCif/Portafolio_Rust.git
git push -u origin main
```

### Opción B: script local (rápido)

Este repo incluye un script local llamado `subir_portafolio.sh` (ignorado por git) para hacer `add/commit/push` en un paso:

```sh
bash subir_portafolio.sh
```
